...
exports = module.exports = function bodyParser(options) {
    options = options || {onPart: customizedOnPart};
...
/////////////////////////
// customize for heroku
/////////////////////////
var maxFileSize = 1048576;//1MB
var customizedOnPart = function(part) {
    var self = this;
    var value = null, size = 0;
    var error = false;
    part.on('data', function(buffer) {
        if (!value) {
            value = buffer;
        } else {
            var temp = new Buffer(size + buffer.length);
            value.copy(temp, 0, 0, size);
            buffer.copy(temp, size, 0, buffer.length);
            value = temp;
        }
        size += buffer.length;
        if (size > maxFileSize) {
            error = new Error('maxFieldsSize ' + maxFileSize + ' exceeded, received ' + size + ' bytes of field data');
            return;
        }
    });
    part.on('end', function() {
        self.emit('field', part.name, {
